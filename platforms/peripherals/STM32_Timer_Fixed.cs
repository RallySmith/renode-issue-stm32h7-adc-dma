// Derived from 1.15.0 STM32_Timer.cs
//
// Add TRGO support
//
...
namespace Antmicro.Renode.Peripherals.Timers
{
    ...
    public class STM32_Timer_Fixed : LimitTimer, IDoubleWordPeripheral, INumberedGPIOOutput, IPeripheralRegister<IGPIOReceiver, NumberRegistrationPoint<int>>, IPeripheralRegister<IGPIOReceiver, NullRegistrationPoint>
    {
        public STM32_Timer_Fixed(Machine machine, long frequency, uint initialLimit) : base(machine.ClockSource, frequency, limit: initialLimit,  direction: Direction.Ascending, enabled: false, autoUpdate: false)
        {
            ...
            {
                this.Log(LogLevel.Debug, "LimitReached: updateDisable {0} masterMode {1}", updateDisable.Value, masterMode.Value);
                if(updateDisable.Value)
                {
                    return;
                }

                if(MasterMode.Update == masterMode.Value)
                {
                    this.Log(LogLevel.Debug, "LimitReached: Update -> TRGO");
                    TRGO.Blink(); // HIGH pulse
                }
            ...
            var registersMap = new Dictionary<long, DoubleWordRegister>
            {
                ...
                {(long)Registers.Control2, new DoubleWordRegister(this)
                    ...
                    .WithEnumField(4, 3, out masterMode, name: "Master mode selection (MMS)")
                ...
                {(long)Registers.SlaveModeControl, new DoubleWordRegister(this)
                    ...
                    .WithTag("TS", 4, 2)
                    .WithTaggedFlag("MSM", 7) // Master/Slave Mode
                    .WithTag("ETF", 8, 3)
                    .WithTag("ETPS", 12, 2)
                ...
                {(long)Registers.EventGeneration, new DoubleWordRegister(this)
                    .WithFlag(0, FieldMode.WriteOneToClear, writeCallback: (_, val) =>
                    {
                        ...
                        // CONSIDER: TRGO generation if MMS==Reset
                    ...
                    // TODO: STM32F2 RM0033rev9 13.4.6: bit7 BG Break Generation
        ...
        public uint ReadDoubleWord(long offset)
        {
            ...
            this.Log(LogLevel.Debug, "Read  0x{0:X} = 0x{1:X}", offset, value);
            return value;
        }

        public void WriteDoubleWord(long offset, uint value)
        {
            this.Log(LogLevel.Debug, "Write 0x{0:X} = 0x{1:X}", offset, value);
        ...
        public GPIO TRGO { get; private set; }
        ...
        {
            Reset         = 0, // TIMx_EGR:UG
            Enable        = 1, // CNT_EN signal
            Update        = 2, // Update event
            ComparePulse  = 3, // CC1IF flag is to be set
            CompareOC1REF = 4, // OC1REF
            CompareOC2REF = 5, // OC2REF
            CompareOC3REF = 6, // OC3REF
            CompareOC4REF = 7, // OC4REF
        }
